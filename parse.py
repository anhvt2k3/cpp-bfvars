...
test_ids = (
    ...
    for filename 
    in os.listdir(".") 
    if "icisn-configuration" in filename
)
...
def get_conf(input_file):
    ...
    with open(input_file, mode='r', newline='\n') as infile:
        ...
        conf = {
            "FilterSize": int(conf_row[0]),
            "Capacity": int(conf_row[1]),
            "NumHashFunctions": int(conf_row[2]),
            "FalsePositiveRate": float(conf_row[3]),
            "NumItemsAdded": int(conf_row[4]),
            "BucketSize": int(conf_row[5]),
            "BucketMaxValue": int(conf_row[6]),
            "BucketCount": int(conf_row[7]),
            "HashAlgo": str(conf_row[8]),
            "HashScheme": str(conf_row[9]),
        }
...
for test_id in test_ids:
    ...
    conf_filename = f"icisn-configuration-{test_id}"
